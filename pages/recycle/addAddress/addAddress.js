...
Page({
    ...
    getSaveAddress: function() {
        var s = this, e = t.globalData.hwxUserInfo.token, a = {};
        a.id = s.data.id, a.user_name = s.data.name, a.mobile = s.data.phone, a.province_id = s.data.province_id, 
        a.city_id = s.data.city_id, a.district_id = s.data.district_id, a.address = s.data.detailAddress, 
        a.is_default = s.data.is_default_num;
        var d = a;
        d.province_name = s.data.province, d.city_name = s.data.city, d.district_name = s.data.district, 
        i.getSaveAddress(e, a, function(i) {
            wx.showToast({
                title: "保存成功",
                icon: "success",
                duration: 1e3
