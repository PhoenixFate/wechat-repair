...
Page({
    ...
    bindChange: function(i) {
        ...
        if (console.log(t, s), t[0] == s[0]) if (t[1] == s[1]) {
            ...
            for (var n = [], o = [], c = 0; c < e[t[0]].cities.length; c++) n.push(e[t[0]].cities[c].name);
            for (var r = 0; r < e[t[0]].cities[0].districts.length; r++) o.push(e[t[0]].cities[0].districts[r].name);
            this.setData({
    ...
    getQuickOrder: function() {
        ...
        this.data.id ? e.user_address_id = this.data.id : e.user_address_id = "hiweixiu", 
        e.user_name = this.data.name, e.phone_number = this.data.phone, e.province = this.data.province_id, 
        e.city = this.data.city_id, e.district = this.data.district_id, e.is_default_address = this.data.is_default_num, 
        e.address_detail = this.data.detailAddress;
        var a = this;
        i.getQuickOrder.call(a, s, e, function(i) {
    ...
    onLoad: function(s) {
        ...
        i.getAllCityData.call(e, function(i) {
            ...
            for (var a = e.data.cityData, d = [], n = [], o = [], c = 0; c < a.length; c++) d.push(a[c].name);
            console.log("省份完成");
            for (var r = 0; r < a[0].cities.length; r++) n.push(a[0].cities[r].name);
            console.log("city完成");
            for (var l = 0; l < a[0].cities[0].districts.length; l++) o.push(a[0].cities[0].districts[l].name);
